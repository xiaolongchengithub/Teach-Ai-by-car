...
class Car(SocketMixin):
    ...
    def __send_order(self, order={ "function": {"auto_run": [8, 1]},
                                    "mode": 1,
                                    "speed": 10,
                                    "time": 10}):
        """通过Tcp ip发送消息
    ...
    @staticmethod
    def demo_line_tracking(speed=8):
        """
       ...
       # ip = input("请输入树莓的IP:")
        ...
        try:
            while True:
                ret = car.line_tracking_turn_type()
                turn = str(ret, encoding="utf-8")
                if turn == 'straight':
